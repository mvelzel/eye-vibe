...
| **D. Irregular 17-bit mask** | The unused actual bit pattern is a selector over 17-symbol slices beginning at the circle-selected final starts. | Freeze forward/reverse and mask/complement only. Compare masked equality skeletons, aligned equality, and partial-bijection consistency to matched body shuffles. | Promote only a corrected held-out structural tail below `.01`; a visually interesting sum fails. |
...
| **G. Diamond corroboration** | The reported diamond radii `24,20,16` independently repeat the two trim lengths and zero-based final address. | Acquire and count the exact asset pixels/cells, establish chronology, and compare its complete geometry rather than the three quoted numbers. | Do not use Discord counts as ground truth or multiply corroborating-number coincidences. |
...
|---|---:|---:|---:|---:|---:|
| D. irregular mask | 4 | 5 | 5 | 4 | **18** |
| G. diamond geometry | 4 | 5 | 4 | 4 | **17** |
...
Lane D is the first prospective cryptographic test because the *content* of
the 17-bit band has not been used to fit the selector. Freeze the exact slices,
orientations, metrics, and matched null before inspecting their scores.

In parallel conceptually, acquire the diamond asset for lane G. It may supply
an independent geometric reason for zero-based address 16. Do not deepen the
factoradic/rune mapping until a rune-to-eye ordering exists outside the Eye
corpus.
